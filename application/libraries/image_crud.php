...
class image_CRUD {
    ...
    protected $normal_prefix = 'normal__';
    protected $large_prefix = 'large__';
    protected $views_as_string = '';
    protected $css_files = array();
    protected $js_files = array();
    //  protected $max_width = 1024;
    // protected $max_height = 768;
    protected $max_width = 5400;
    protected $max_height = 3700;
    ...
    protected $default_language_path = 'image_crud/languages';
    // custom
    protected $action_url = null;
    protected $watermark_text = null;
    protected $watermark_font = null;
    protected $watermark_img = null;
    ...
    // add custom
    public function set_actionUrl($url) {
        $this->action_url = $url;
    }
    ...
    public function set_watermark_font($font) {
        $this->watermark_font = $font;
    }

    public function set_watermark_img($image) {
        $this->watermark_img = $image;
    }
    ...
    protected function _upload_file($upload_dir) {
        ...
        $uploader_response = $upload_handler->post(); // upload the image
...
// Delete Large File
        if (!file_exists($this->image_path . '/' . $this->large_prefix . $row->{$this->url_field})) {
            unlink($this->image_path . '/' . $this->large_prefix . $result->{$this->url_field});
        }
        // Delete Normal File
        if (!file_exists($this->image_path . '/' . $this->normal_prefix . $row->{$this->url_field})) {
            unlink($this->image_path . '/' . $this->normal_prefix . $result->{$this->url_field});
        }

        if ($this->table_name == 'shop_brands' && $this->relation_field) {
            $this->ci->load->model('shop_model');
            $this->ci->shop_model->deleteShopModel($id);
        }
    ...
    protected function _get_delete_url($value) {
        ...
        $action_url = $this->action_url == null ? $rsegments_array[2] : $this->action_url;
        return site_url($rsegments_array[1] . '/' . $action_url . '/delete_file/' . $value);
    }
    ...
    protected function _convert_foreign_characters($str_i) {
        include('image_crud/config/translit_chars.php');
    ...
    protected function _create_normal($image_path, $thumbnail_path) {
        $this->image_moo
                ->load($image_path)
                ->resize_crop(450, 300)
                ->save($thumbnail_path, true);
    ...
    protected function _create_large($image_path, $thumbnail_path) {
        $this->image_moo
                ->load($image_path)
                ->resize_crop(740, 460)
                ->save($thumbnail_path, true);
    ...
    protected function _insert_watermark($image_path) {
        $this->image_moo
                ->load($image_path)
                ->make_watermark_text($this->watermark_text, $this->watermark_font)
                ->resize_crop(740, 460)
                ->watermark(9, 50)
                ->save($image_path, TRUE);
    }

    protected function _add_verified($image_path) {
        $this->image_moo
                ->load($image_path)
                ->load_watermark($this->watermark_img)
                ->resize_crop(740, 460)
                ->watermark(9, 10)
                ->save($image_path, TRUE);
    }
    ...
    /*
     * Update customize the controller page url
     * @rsegments_array[2] => controller class 
     * $this->action_url
     */
    ...
    protected function getState() {
        ...
        if (isset($rsegments_array[3]) && is_numeric($rsegments_array[3])) {
            $upload_url = site_url($rsegments_array[1] . '/' . $action_url . '/upload_file/' . $rsegments_array[3]);
            $ajax_list_url = site_url($rsegments_array[1] . '/' . $action_url . '/' . $rsegments_array[3] . '/ajax_list');
            $ordering_url = site_url($rsegments_array[1] . '/' . $action_url . '/ordering');
            $insert_title_url = site_url($rsegments_array[1] . '/' . $action_url . '/insert_title');
            ...
            return (object) $state;
        } elseif (isset($rsegments_array[3]) && is_string($rsegments_array[3]) && !in_array($rsegments_array[3], array('upload_file', 'ordering', 'insert_title', 'delete_file', 'ajax_list'))) {
            $upload_url = site_url($rsegments_array[1] . '/' . $action_url . '/upload_file/' . $rsegments_array[3]);
            $ajax_list_url = site_url($rsegments_array[1] . '/' . $action_url . '/' . $rsegments_array[3] . '/ajax_list');
            $ordering_url = site_url($rsegments_array[1] . '/' . $action_url . '/ordering');
            $insert_title_url = site_url($rsegments_array[1] . '/' . $action_url . '/insert_title');
            ...
            $upload_url = site_url($rsegments_array[1] . '/' . $action_url . '/upload_file');
            $ajax_list_url = site_url($rsegments_array[1] . '/' . $action_url . '/ajax_list');
            $ordering_url = site_url($rsegments_array[1] . '/' . $action_url . '/ordering');
            $insert_title_url = site_url($rsegments_array[1] . '/' . $action_url . '/insert_title');
            ...
            $old_file_name = isset($_GET['qqfile']) && !empty($_GET['qqfile']) ? $this->_convert_foreign_characters($_GET['qqfile']) : '';
            ...
            if (isset($rsegments_array[4]) && (is_numeric($rsegments_array[4]) || is_string($rsegments_array[4]))) {
            ...
            switch ($state_info->name) {
                ...
                case 'upload_file':
                    ...
                    if(!is_dir($this->image_path)) {
                        mkdir($this->image_path, 0744, true);
                        file_put_contents($this->image_path."/index.html", "");
                    } 
                    $file_name = $this->_upload_file($this->image_path);

                    if ($file_name !== false) {
                        if ($this->table_name == 'shop_photos') {

                            // ceate large
                            //  $this->_create_large($this->image_path . '/' . $file_name, $this->image_path . '/' . $this->large_prefix . $file_name);
                            // normal 
                            $this->_create_normal($this->image_path . '/' . $file_name, $this->image_path . '/' . $this->normal_prefix . $file_name);
                            // create thumbnail
                            $this->_create_thumbnail($this->image_path . '/' . $file_name, $this->image_path . '/' . $this->thumbnail_prefix . $file_name);
                        }
                        if (isset($state_info->relation_value) && !empty($state_info->relation_value))
                            $this->_insert_table($file_name, $state_info->relation_value);
                        else
                            $this->_insert_table($file_name);
